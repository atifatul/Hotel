...
// Props:
// allPackages - API se mila saara data (dropdowns banane ke liye)
// onSearch - Ek function jo search criteria (destination, theme, duration) ko parent component tak bhejega
// isLoading - Taki data load hone tak dropdown disable dikhe
//
const ActivitySearchFilter = ({ allPackages, onSearch, isLoading }) => {
  // --- States for Dropdown Lists ---
  const [allDestinations, setAllDestinations] = useState([]);
  const [availableThemes, setAvailableThemes] = useState([]);
  const [availableDurations, setAvailableDurations] = useState([]); // <-- NAYA STATE

  // --- States for User Selections ---
  const [selectedDestination, setSelectedDestination] = useState(null);
  const [selectedTheme, setSelectedTheme] = useState(null);
  const [selectedDuration, setSelectedDuration] = useState(null); // <-- NAYA STATE

  // --- States for Dropdown controls ---
  const [isDestinationOpen, setIsDestinationOpen] = useState(false);
  const [isThemeOpen, setIsThemeOpen] = useState(false);
  const [isDurationOpen, setIsDurationOpen] = useState(false); // <-- NAYA STATE
  const [destinationSearchTerm, setDestinationSearchTerm] = useState("");

  // --- Refs for dropdowns to handle outside clicks ---
  const destinationRef = useRef(null);
  const themeRef = useRef(null);
  const durationRef = useRef(null); // <-- NAYA REF

  // 1. Jab 'allPackages' prop parent se mile, toh destinations ki list banayein
  useEffect(() => {
  ...
  // 2. Jab user Destination select kare, toh usse judi Themes ki list banayein
  useEffect(() => {
    ...
    setSelectedTheme(null); // Destination badalne par Theme reset karein
    setSelectedDuration(null); // Aur Duration bhi reset karein
  }, [selectedDestination, allPackages]);

  // 3. JAB DESTINATION AUR THEME, DONO SELECT HO, TAB DURATION LIST BANAYEIN (NAYA EFFECT)
  useEffect(() => {
    if (selectedDestination && selectedTheme) {
      ...
      allPackages
        .filter(
          (pkg) =>
            ...
            pkg.days && // Ensure days/nights exist
            pkg.nights
        )
        .forEach((pkg) => {
      ...
      setAvailableDurations([]); // Agar theme ya dest select nahi hai toh list khali karein
    }
    setSelectedDuration(null); // Theme badalne par Duration reset karein
  }, [selectedDestination, selectedTheme, allPackages]);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // Duration ref ke liye check add karein
  ...
  // Naya handler duration ke liye
  const handleDurationSelect = (duration) => {
    setSelectedDuration(duration);
    setIsDurationOpen(false);
  };

  // Destination dropdown ke liye search logic
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // 'onSearch' prop ko call karein aur selected values parent ko bhej dein
    onSearch({
      destination: selectedDestination,
      theme: selectedTheme,
      duration: selectedDuration, // <-- DURATION KO BHI BHEJEIN
    });
  };
      ...
      <div className="container">
        <div className="filter-input-wrap">
          ...
          <form className="filter-input show" onSubmit={handleSubmit}>
            {/* Destination Dropdown (No change) */}
            <div className="single-search-box" ref={destinationRef}>
              ...
              <div
                ...
                onClick={() =>
                  !isLoading && setIsDestinationOpen(!isDestinationOpen)
                }
              >
                <input
                  type="text"
                  ...
                  value={
                    isLoading
                      ? "Loading..."
                      : selectedDestination || "Where are you going?"
                  }
                  disabled={isLoading}
                  style={{ display: "block" }}
                />
            ...
            {/* Activity/Theme Dropdown (No change) */}
            <div className="single-search-box" ref={themeRef}>
              ...
              <div
                ...
                onClick={() =>
                  !isLoading &&
                  selectedDestination && // <-- Theme ko click karne ke liye destination zaroori hai
              ...
              >
                <input
                  ...
                  disabled={isLoading || !selectedDestination} // <-- Jab tak destination select na ho, disable rakhein
                />
              </div>
              <div
              ...
              >
                <ul className="option-list">
                  {availableThemes.length > 0 ? (
                    ...
                    <li className="single-item" style={{ color: "#888" }}>
                      <h6>
                        {isLoading
                          ? "Loading..."
                          : !selectedDestination
            ...
            {/* ========== NAYA DURATION DROPDOWN (DATE KI JAGAH) ========== */}
            <div className="single-search-box" ref={durationRef}>
              {/* Maine calendar wala SVG use kiya hai */}
              <svg
                width="18"
                height="18"
                viewBox="0 0 18 18"
              ...
              <div
                ...
                onClick={() =>
                  !isLoading &&
                  selectedTheme && // <-- Jab tak theme select na ho, click disable rakhein
              ...
              >
                <input
                  ...
                  disabled={isLoading || !selectedTheme} // <-- Jab tak theme select na ho, disable rakhein
                />
              </div>
              <div
                className={`custom-select-wrap two no-scroll ${
                  isDurationOpen ? "active" : ""
                }`}
              >
                <ul className="option-list">
