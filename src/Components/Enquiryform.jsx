...
const EnquiryForm = ({ price, packageName }) => {
  ...
  return (
    ...
    <div className="widget-card mb-30">
      {" "}
      {/* Use existing widget style or create new */}
      <div className="widget-body">
        <div className="package-sidebar-price mb-3 text-center">
          <span>Starting From</span>
          <h4>
            ₹{price || "N/A"} <small>/ Per Person</small>
          </h4>
        </div>
        <h5 className="package-sidebar-title text-center mb-4">
          {packageName}
        </h5>
        ...
        <form className="booking-form" onSubmit={handleSubmit}>
          ...
          <div className="row g-3 mb-3">
            {" "}
            {/* Use row for side-by-side on larger screens if needed */}
          ...
          <button
            ...
            style={{ width: "100% ", justifyContent: "center" }} // Use button style and make full width
            disabled={isSubmitting} // Disable while submitting
          >
            {isSubmitting ? "Sending..." : "Send Enquiry"}
          </button>
          {message && (
            <p className="mt-3 text-center" style={{ fontSize: "0.9em" }}>
              {message}
            </p>
          )}
        </form>
